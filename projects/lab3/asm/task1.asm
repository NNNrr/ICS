...
addi    x9,     zero,   8   ; set loop index = 8

addi    x10,    zero,   0   ; first loop begin

addi    x5,      x1,    0   ; addr_A = A_baseaddr
addi    x11,    zero,   0   ; second loop begin

addi    x12,    zero,   0   ; third loop begin
...
addi    x12,    x12,    1   
bne     x12,    x9 ,    -32 ; thrid loop check

lw      x18,    0(x7)
add     x17,    x18,    x17 ; reg[17] = row(A)*col(B) + C
sw      x17,    0(x8)       ; save x17 into mem[x8] 
addi    x17,    zero,   0   ; clear reg[17]
addi    x7,     x7,     4   ; addr_c = addr_C + 4
addi    x8,     x8,     4   ; addr_D = addr_D + 4

addi    x11,    x11,    1   
bne     x11,    x9 ,    -72 ; second loop check

addi    x6,     x6,     32  ; reg[x6] = reg[x6] + 32 // addr_col(B) + 1

addi    x10,    x10,    1   
bne     x10,    x9 ,    -92 ; the first loop check